...
## 网页的基本布局
先简答介绍一下何为盒子模型,组件式的逻辑与这个具有强关联,在html文档中标明一个class属性(假设class = "box-A"),接下来在css中引用他给他设计属性,对于基础属性宽高，盒子模型将其分为三层。第一层是内容区，显示你的文字，svg图标等等,第二层是内边距区,即padding,边界的一个外扩，可以让按钮的可触面积变大，让字体在更大的可显示空间,
一般情况都要使用他，第三层是外边距，即是margin,标记距离下一个盒子模型的距离，当两个盒子模型相邻的时候一个他们之间的间距不是外边距的叠加，而是取两个之中更大值。而边框border则是纯纯的修饰性属性,可以把他看作文字的描边属性,突出显示
\`\`\`css
  .box-A {
  width: 100px;
  height: 40px;
  ...
  border-radius: 8px;
\`\`\`
接下来就是怎么在网页中布局这个难点了,怎么设计呢?我在这里就拿我的音乐播放器举例,可以看到很明显的分区
